...
<html>
				...
				<!-- Main -->
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" enctype="multipart/form-data" method="post">
                        <div class="form-group <?php echo (!empty($name_err)) ? 'has-error' : ''; ?>">
                            <label>Text</label>
                            <br>
                            <input type="text" onkeyup="showHint(this.value)" type="text" name="name" class="form-control" value="<?php echo $name; ?>">
                            <span class="help-block"><?php echo $name_err; ?></span>
                        </div>   
                        <p>Example: <span id="txtHint"></span></p>
	...
	$url = 'http://localhost/Sendimage/controller/server.php';
	$curl = curl_init($url);
	curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
	curl_setopt($curl, CURLOPT_POST, false);
	$curl_odgovor = curl_exec($curl);
	curl_close($curl);
	$states = new SimpleXMLElement($curl_odgovor,null,false);
	if (property_exists($states,"greska")){
	echo ($states->greska);
	} else {
	// ako nema greske, generise se tabela
	?>
	<h2>Where you can sand images</h2>
	<table id="customers">
	<tr>
	<td>ID</td>
	<td>Names</td>
	...
	?>
	<tr>
	<td><?php echo $p->idState;?></td>
	<td><?php echo $p->nameState;?></td>
	</tr>
	<?php
	}
	?>
